...
if (!isAdmins) return reply(ADMIN)	
  if (args[1] == 'second') {
                ...
                setTimeout(() => {
                    ...
                    const open = `*OPEN TIME* THE GROUP WAS OPENED BY TERRA_MD TO APPROVED ADMIN\n NOW MEMBERS CAN SEND MESSAGES 🔓`
...
if (!isAdmins) return reply(ADMIN)	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY TERRA_MD AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
if (!isBotAdmins) return reply(mg.needbotadmins);     
           ...
           const mass = await conn.sendMessage(m.chat, { text: '*GROUP CHAT MUTED BY TERRA_MD* 🔒' }, { quoted: mek });
...
if (!isBotAdmins) return reply(mg.needbotadmins);     
           ...
           const mass = await conn.sendMessage(m.chat, { text: '*GROUP CHAT UNMUTED BY TERRA_MD* 🔒' }, { quoted: mek });
            return await conn.sendMessage(m.chat, { react: { text: '🔒', key: mass.key } });
} catch(e) {
console.log(e);
reply('*PLEASE GIVE ME A ADDMIN ROLE❗👻*')    
